...
static long simple_spin_ioctl(struct file *file, unsigned int cmd, unsigned long arg) {
	...
	switch(cmd) {
		case SIMPLE_SPIN_READ:
			...
			printk("simple spin: ioctl read return %d\n", ret);
			break;
		case SIMPLE_SPIN_WRITE:
			ret = simple_spin_write(user_buf);
			printk("simple spin: ioctl write return %d\n", ret);
			break;
		default:
			return -1;
	}
...
static int __init simple_spin_init(void) {
	...
	printk("simple spin: INIT MODULE\n");
	...
	if(ret < 0) {
		printk("FAILED TO ADD A CHARACTER DEVICE\n");
		return -1;
	}
...
static void __exit simple_spin_exit(void) {
	printk("simple spin: EXIT MODULE\n");
	cdev_del(cd_cdev);
